...
{
    class AmmoHeavyPistol : Item
    {

        public AmmoHeavyPistol()
        {
            Id = 39;
            Name = "HeavypistolAmmo";
            ImagePath = "AmmoHeavyPistol.png";
            WeightInG = 250;
            MaxStackSize = 25;
        }
		...
		{
			if (NAPI.Player.GetPlayerCurrentWeapon(p) == WeaponHash.HeavyPistol)
			{
				...
				NAPI.Task.Run(delegate
				{
					...
					Database.changeInventoryItem(p.Name, "AmmoHeavyPistol", 1, true);
					Notification.SendPlayerNotifcation(p, "Du hast 222 Kugeln in deine Heavy Pistol gefüllt", 4500, "green", "WAFFE", "");
					p.TriggerEvent("disableAllPlayerActions", new object[1]
					{
							false
					});
			...
			else
			{
				...
				Database.changeInventoryItem(p.Name, "AmmoHeavyPistol", 1, false);
			}
			return true;
		}
	}
