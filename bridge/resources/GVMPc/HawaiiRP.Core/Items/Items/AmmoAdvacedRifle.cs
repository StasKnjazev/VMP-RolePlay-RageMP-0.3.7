...
{
	class Ammoadvancedrifle : Item
	{

		public Ammoadvancedrifle()
		{
			Id = 19;
			Name = "Ammoadvancedrifle";
		...
		{
			if (NAPI.Player.GetPlayerCurrentWeapon(p) == WeaponHash.AdvancedRifle)
			{
				...
				NAPI.Task.Run(delegate
				{
					...
					Database.changeInventoryItem(p.Name, "Ammoadvacedrifle", 1, true);
					Notification.SendPlayerNotifcation(p, "Du hast 222 Kugeln in deine Advancedrifle gefüllt", 4500, "grey", "", "");
					p.TriggerEvent("disableAllPlayerActions", new object[1]
					{
							false
					});
			...
			} else
			{
				...
				Database.changeInventoryItem(p.Name, "Ammoadvacedrifle", 1, false);
			}
			return true;
		}
	}
